"""RIZIN.54 勝敗予想まとめ 時間分割レンダーの窓境界を安全な位置に選ぶ。
境界は「字幕(キュー)の切れ目」かつ「章/予想者ラベル/カットアウト/Xカード/集計の外」に置く。
...
HTML = (ROOT / "hyperframes" / "templates" / "rizin54-yosou" / "index.html").read_text(encoding="utf-8")
...
for key in ("plabels", "pcuts", "xcards", "tally"):
    for c in D.get(key, []):
