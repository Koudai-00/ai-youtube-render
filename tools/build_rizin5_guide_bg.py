...
PLAN = {
    ...
    "o3": PR("openworkout", 700),                 # ★出場選手が話している様子(B1:アナウンサーを回避)
    ...
    "f1c": FT("usami_intro2", 4047),              # ★B2 宇佐美インタビュー(1:07:27〜)
    "f1d": PR("presser2", 520),                   # 会見(武尊推薦/兄の話)
    "f1e": FT("beynoah_recent", 40),              # ベイノア 名前コール+プロフィール(現状維持)
    "f1f": FT("beynoah_recent", 55),              # ★B4 前戦の試合映像(ダウン場面/古いRISE映像を回避)
    ...
    "f4f_2": SP("saito_intro", 217, "yaman_other", 873),  # ★B18 2分割(12:28〜)
