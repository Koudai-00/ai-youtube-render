...
DISP = {
    ...
    "かわはらゆうじ": "河原由次", "ジャンさいとう": "ジャン斉藤", "ぐしけんようこう": "具志堅用高",
...
CARDS_SPEC = [
    # ★実名で出すのは選手・元選手・解説者・報道のみ。一般アカウントは載せない。
    ("c1h", "media", "ジョビン", "今までの判定で1番びっくりしたかも。平本自身が負けた顔してたもん。これはまずいんじゃない?",
     "元DEEPフェザー級王者／実況配信"),
    ("c1i", "media", "ジョビン", "トータルマストなら100%ダウトベック。ただ、ラウンドマストならあり得る判定ですね。",
     "元DEEPフェザー級王者／実況配信"),
    ("c1j", "media", "河原由次", "人気がある選手だから。スターだから。もし、そんなものが1%でも判定に入るなら、もうスポーツじゃない。",
     "サムライソウルCOO"),
    ("c1k", "media", "ジャン斉藤", "1R ダウトベック、2R 平本蓮。3Rのダウトベックのポジションキープをどう見るか。手数は平本蓮だった。",
     "格闘技ライター"),
    ("c1p", "media", "平本 蓮", "1R目は10-8取られるかと思ったけど、2・3取ってれば28-28。トータルじゃなくてラウンドマストだから大丈夫、とセコンドに言われた。",
     "試合後インタビューより"),
    ("c1s", "media", "カルシャガ・ダウトベック", "自分が負けたとは思っていません。1Rで拳を折った感覚があった。それでも1Rと3Rは自分が取ったと理解しています。",
     "試合後インタビューより"),
    ("c2f", "media", "朝倉 未来", "これが日本の格闘技の歴史の1ページです。俺はこの経験を経て先に進みたいと思います。",
     "勝利者マイクより"),
    ("c2g", "media", "青木 真也", "この通りしっかりやられたから。今日は彼の方が強かった。それ以上でもそれ以下でもなし。乾杯。",
     "試合後インタビューより"),
]
